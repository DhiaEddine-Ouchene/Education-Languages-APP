...
import { Eye, PlayCircle, Save, Settings, Sparkles } from "lucide-react";
...
export function GameBuilder({ sets, initial }: Props) {
  ...
  return (
    <div className="space-y-4">
      <Card className="border-primary-light bg-gradient-to-r from-white to-primary-light/50">
        <CardContent className="pt-4">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-end">
            <div className="flex-1 min-w-[220px]">
              <Label>Game title</Label>
              <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Fruit vocabulary quiz" />
              <FieldError message={error} />
            </div>
            <div className="flex flex-wrap gap-2">
              <Button variant={mode === "build" ? "primary" : "outline"} type="button" onClick={() => setMode("build")}>
                <Settings className="h-4 w-4" /> Builder
              </Button>
              <Button variant={mode === "try" ? "accent" : "outline"} type="button" onClick={() => setMode("try")}>
                <PlayCircle className="h-4 w-4" /> Try current setup
              </Button>
              <Button onClick={save} disabled={saving}>
                <Save className="h-4 w-4" /> {saving ? "Saving..." : initial ? "Save" : "Create game"}
              </Button>
              {initial && (
                <Link href={`/dashboard/games/${initial.id}/preview`}>
                  <Button variant="outline" type="button"><Eye className="h-4 w-4" /> Full preview</Button>
                </Link>
              )}
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[260px_1fr_280px] gap-4">
          <Card><CardContent className="pt-4">
            ...
            <ul className="mt-3 space-y-1 max-h-72 overflow-y-auto pr-1">
              {selectedSet?.items.map((i) => (
                <li key={i.id} className="text-xs border border-border rounded-btn px-2 py-1.5 flex justify-between gap-2 bg-background"><span className="font-medium">{i.word}</span><span className="text-txt-secondary text-right">{i.translation}</span></li>
              ))}
          ...
          <Card><CardContent className="pt-4">
            <div className="mb-4 flex items-center justify-between gap-3">
              <div>
                <p className="font-heading font-semibold">Game experience</p>
                <p className="text-xs text-txt-secondary">Choose a polished activity type, then test it instantly.</p>
              </div>
              <span className="inline-flex items-center gap-1 rounded-pill bg-accent-light px-3 py-1 text-xs font-semibold text-accent"><Sparkles className="h-3.5 w-3.5" /> Pro preview</span>
            </div>
            <div className="flex flex-wrap gap-1.5 mb-4">
              {GAME_TYPES.map((t) => (
                <button key={t.key} onClick={() => setType(t.key)} className={cn("px-3 py-2 rounded-btn border text-xs transition-all", type === t.key ? "border-primary bg-primary-light text-primary-dark font-semibold shadow-card" : "border-border text-txt-secondary hover:border-primary-light hover:bg-background")}>
                  {t.emoji} {t.label}
                </button>
              ))}
            </div>
            <div className="border border-dashed border-border rounded-card p-6 text-center bg-background">
              <p className="text-xs text-txt-secondary mb-3">Quick visual preview</p>
              {preview ? (
                ...
                : type === "QUIZ" || type === "FILL_BLANK" || type === "SPEED_ROUND" ? (
                  <div><p className="font-heading font-semibold mb-3">What does “{preview.word}” mean?</p>
                    <div className="grid grid-cols-2 gap-2 max-w-xs mx-auto">{[preview.translation, "Option B", "Option C", "Option D"].map((o, i) => <div key={i} className="border border-border rounded-btn py-2 text-sm bg-card">{o}</div>)}</div></div>
                ) : type === "MEMORY" ? (
                  <div className="grid grid-cols-4 gap-2 max-w-xs mx-auto">{Array.from({ length: 8 }).map((_, i) => <div key={i} className="h-12 bg-primary rounded-btn shadow-card" />)}</div>
              ...
              <Button className="mt-5" variant="accent" type="button" onClick={() => setMode("try")}><PlayCircle className="h-4 w-4" /> Try this game now</Button>
            </div>
          </CardContent></Card>
