...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (student && cls._count.members === 0) void sendFirstStudentJoined(cls.educator.user.email, cls.name, student.name);
    return NextResponse.json({ className: cls.name }, { status: 201 });
  } catch (err) {
